...
void Kinect2StreamImpl::createFrameBuffer()
{
  //if (m_sensorType == ONI_SENSOR_COLOR && !m_pFrameBuffer.color) {
	 // m_pFrameBuffer.color = new RGBQUAD[512 * 424];
  //}
  ...
  //}
	if (m_sensorType == ONI_SENSOR_COLOR && !m_pFrameBuffer.color) {
		m_pFrameBuffer.color = new RGBQUAD[640 * 480];
	}
	else if (m_sensorType == ONI_SENSOR_DEPTH && !m_pFrameBuffer.depth) {
		m_pFrameBuffer.depth = new UINT16[640 * 480];
	}
	else if (!m_pFrameBuffer.infrared) { // ONI_SENSOR_IR
		m_pFrameBuffer.infrared = new UINT16[640 * 480];
	}
}
  ...
  if (m_sensorType == ONI_SENSOR_COLOR) {
	  if (m_pFrameReader.color && m_pFrameBuffer.color) {
		  ...
		  if (SUCCEEDED(hr)) {
			  ...
			  if (SUCCEEDED(hr))
			  {
				  ...
				  RGBQUAD* data1 = new RGBQUAD[512 * 363];
				  frame->CopyConvertedFrameDataToArray(1920 * 1080 * sizeof(RGBQUAD), reinterpret_cast<BYTE*>(data), ColorImageFormat_Bgra);
				  
				  Registration reg;
				  reg.apply(data, data1);
				//  memcpy(m_pFrameBuffer.color, data2, 512 * 424 * sizeof(RGBQUAD));
				  resize<RGBQUAD>(data1, m_pFrameBuffer.color,512,363,640,480);
				  delete[] data;
				  delete[] data1;
				  data = NULL;
				  data1 = NULL;
    ...
    if (m_pFrameReader.depth && m_pFrameBuffer.depth) {
      ...
      if (SUCCEEDED(hr)) {
		...
		UINT16* data1 = new UINT16[512 * 424];
		UINT16* data2 = new UINT16[512 * 363];
		memcpy(data1, data, 512 * 424 * sizeof(UINT16));
		Registration reg;

		reg.apply(data1, data2);
		resize<UINT16>(data2, m_pFrameBuffer.depth, 512, 363, 640, 480);
		delete[] data1;
		delete[] data2;
		data2 = NULL;
		data1 = NULL;
    ...
    if (m_pFrameReader.infrared && m_pFrameBuffer.infrared) {
      ...
      if (SUCCEEDED(hr)) {
		...
		reg.apply(data1, data2);
		resize<UINT16>(data2, m_pFrameBuffer.infrared, 512, 363, 640, 480);
		delete[] data1;
		delete[] data2;
		data2 = NULL;
		data1 = NULL;
